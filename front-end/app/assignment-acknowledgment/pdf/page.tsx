...
function AssignmentAcknowledgmentPdfContent() {
  ...
  useEffect(() => {
    ...
    void fetchAssignmentAcknowledgmentPdfRequest(token)
      ...
      .catch((error) => {
        if (!live) return;
        setPdfPayload(null);
        setNotice(error instanceof Error ? error.message : "Failed to load signed PDF.");
      })
      .finally(() => {
        if (!live) return;
        setIsLoading(false);
  ...
  return (
    <main className="min-h-screen bg-[#f1f5f9] px-4 py-8 text-slate-900">
      <section className="mx-auto w-full max-w-[1080px] rounded-[18px] border border-[#dbe4ee] bg-white p-5 shadow-[0_14px_36px_rgba(15,23,42,0.08)] md:p-6">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <button
            ...
            className="h-10 rounded-[10px] bg-[#0f172a] px-4 text-[14px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        {isLoading ? (
          ...
          <iframe
            ...
            src={pdfObjectUrl}
            className="h-[75vh] w-full rounded-[12px] border border-[#dbe4ee]"
          />
        ) : (
        ...
        {notice ? (
          <p className="mt-4 rounded-[10px] border border-[#dbe4ee] bg-[#f8fafc] px-4 py-3 text-[13px] text-[#1e293b]">
            {notice}
          </p>
        ) : null}
      </section>
...
export default function AssignmentAcknowledgmentPdfPage() {
  return (
    <Suspense
      fallback={
        <main className="min-h-screen bg-[#f1f5f9] px-4 py-8 text-slate-900">
          <section className="mx-auto w-full max-w-[1080px] rounded-[18px] border border-[#dbe4ee] bg-white p-6 shadow-[0_14px_36px_rgba(15,23,42,0.08)]">
