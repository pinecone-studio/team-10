    ...
    <section className="w-full">
      <div className="grid gap-4 md:grid-cols-2">
        {props.items.map((item) => (
          <article key={`${item.id}-${item.distributionId ?? "storage"}`} className="flex h-[121px] items-start justify-between justify-self-stretch rounded-[12px] border border-[#D8E8FF] bg-[rgba(255,255,255,0.70)] px-3 py-6">
            ...
            <button
              ...
              className="flex h-[36px] w-[72px] shrink-0 items-center justify-center gap-2 self-start rounded-[6px] bg-[#0F172A] text-[15px] font-medium text-white"
            >
