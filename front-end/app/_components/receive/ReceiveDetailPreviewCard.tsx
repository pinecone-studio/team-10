...
export function ReceiveDetailPreviewCard(props: {
  ...
  return (
    <>
        ...
        <div className="mt-[12px] min-h-0 flex-1 space-y-[12px] overflow-y-auto pr-1">
          ...
          <div className="mt-4 rounded-[12px] border border-[#dbe3ee] bg-[#f8fbff] p-4">
            <p className="text-[13px] font-semibold text-[#0f172a]">
              QR code, serial number, and link
            </p>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="w-full rounded-[12px] border border-[#e2e8f0] bg-[#f8fbff] px-3 py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={props.onOpenQrLink}
                      className="cursor-pointer text-[11px] font-semibold text-[#2563eb] underline underline-offset-2"
                    >
                      Open
                    </button>
                  </div>
                  <button
                    type="button"
                    onClick={props.onCopyQrLink}
                    className="mt-2 block w-full cursor-pointer break-all text-left text-[11px] leading-5 text-[#475569] hover:text-[#2563eb]"
                  >
      ...
      {isQrOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(15,23,42,0.55)] p-6">
          <button
            type="button"
            onClick={() => setIsQrOpen(false)}
            className="inline-flex cursor-zoom-out"
          >
            <BrandedQrCode
              value={props.qrValue}
              title={props.qrTitle}
              size={320}
              className="p-4 shadow-[0_24px_60px_rgba(15,23,42,0.22)]"
              showValue={false}
            />
          </button>
        </div>
