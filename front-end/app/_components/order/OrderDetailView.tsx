...
function VerticalDots() {
  return (
    <svg
      viewBox="0 0 16 16"
      fill="none"
      className="h-[18px] w-[18px] text-[#4a6a95]"
      aria-hidden="true"
    >
...
export function OrderDetailView(props: {
  ...
  return (
    <>
      <TopBar
        actionLabel="Create additional note"
        onAction={props.onCreateNote}
      />
      <h2 className="pt-[6px] text-[32px] font-semibold leading-[1.2] text-[#111111]">
        Order detail
      </h2>

      <section className="pt-[8px]">
        <div className="grid grid-cols-[0.8fr_1.2fr_1fr_1fr_1fr_1fr_0.7fr] gap-[12px] rounded-[6px] border border-[#d7d7da] bg-[#dbdcdf] px-[16px] py-[18px] text-[14px] text-[#707070]">
          <span>Id</span>
          <span>Name</span>
        ...
        <div className="mt-[14px] grid grid-cols-[0.8fr_1.2fr_1fr_1fr_1fr_1fr_0.7fr] items-center gap-[12px] rounded-[6px] border border-[#d7d7da] bg-[#efefef] px-[16px] py-[17px] text-[14px] text-[#565656]">
          ...
          <span
            className={`inline-flex w-fit rounded-[99px] border px-[8px] py-[1px] text-[14px] ${presentation.tone}`}
          >
            {presentation.status.includes("Waiting")
              ? "Waiting"
              : presentation.status.includes("Rejected")
      ...
      <h3 className="pt-[18px] text-[24px] font-semibold leading-[1.2] text-[#111111]">
        Feed
      </h3>
      ...
      <section className="rounded-[10px] px-[12px] pb-[8px]">
        <div className="space-y-[12px]">
          {feedEvents.map((event, index) => (
            <div
              ...
              className="grid grid-cols-[16px_auto_1fr] items-start gap-[10px] text-[14px] text-[#666666]"
            >
              <span
                className={`mt-[6px] inline-flex h-[12px] w-[12px] rounded-full border ${event.featured ? "border-[#888] bg-[#efefef]" : "border-[#b8b8b8]"}`}
              />
        ...
        <div className="mt-[16px] ml-[32px] rounded-[18px] border border-[#d1d2d7] bg-[#efefef] px-[20px] py-[18px] text-[14px] text-[#757575]">
          Write a detailed description
        </div>

        <div className="mt-[20px] ml-[20px] rounded-[10px] border border-[#d7d7da] bg-[#efefef] px-[14px] py-[12px]">
          <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.9fr_0.9fr] gap-[12px] border-b border-[#d4d4d8] pb-[10px] text-[12px] text-[#7a7a7a]">
          ...
          <div className="space-y-[6px] pt-[8px]">
            {props.order.items.map((item, index) => (
              <div
                ...
                className="grid grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.9fr_0.9fr] gap-[12px] py-[6px] text-[14px] text-[#444444]"
              >
                <span>{item.name}</span>
                <span>{item.code}</span>
