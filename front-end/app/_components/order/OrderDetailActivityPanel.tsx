    ...
    <section className="flex h-[650px] flex-col rounded-[20px] border border-[#e2efff] bg-white p-4 shadow-[0_14px_34px_rgba(125,170,232,0.12),0_6px_16px_rgba(15,23,42,0.05)]">
      ...
      <div className="pt-6">
        ...
        <div className="mt-4 flex items-center justify-end gap-3">
          <button type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-[10px] border border-[#dbeafb] bg-white text-lg text-[#111827] transition duration-150 hover:bg-[#f8fafc] active:scale-[0.98] active:bg-[#eef2f7] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c7d2fe] focus-visible:ring-offset-2">✎</button>
          <button type="button" onClick={() => { const nextComment = commentText.trim(); if (!nextComment) return; setLocalComments((current) => [...current, nextComment]); setCommentText(""); }} className="inline-flex h-9 items-center justify-center rounded-[10px] bg-[#111827] px-5 text-sm font-medium text-white transition duration-150 hover:bg-[#1f2937] active:scale-[0.98] active:bg-[#0f172a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c7d2fe] focus-visible:ring-offset-2">Send comment</button>
        </div>
      </div>
    </section>
  );
