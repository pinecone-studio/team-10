    ...
    <section className="rounded-[18px] border border-[#d9e0e8] bg-white p-4">
      ...
      <div className="border-t border-[#eef2f6] pt-5">
        ...
        <div className="mt-4 flex items-center justify-end gap-3">
          <button type="button" className="inline-flex h-11 w-11 items-center justify-center rounded-[10px] border border-[#d9e0e8] bg-white text-lg text-[#111827]">+</button>
          <button type="button" onClick={() => { const nextComment = commentText.trim(); if (!nextComment) return; setLocalComments((current) => [...current, nextComment]); setCommentText(""); }} className="inline-flex h-11 items-center justify-center rounded-[10px] bg-[#111827] px-5 text-sm font-medium text-white">Send comment</button>
        </div>
      </div>
    </section>
  );
