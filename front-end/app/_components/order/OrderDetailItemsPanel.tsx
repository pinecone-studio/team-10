    ...
    <section className="rounded-[18px] border border-[#d9e0e8] bg-white p-4">
      ...
      <div className="mt-4 overflow-hidden rounded-[14px] border border-[#e2e8f0]">
        ...
        <div className="divide-y divide-[#eef2f6]">
          {order.items.map((item, index) => <div key={`${item.catalogId}-${index}`} className="grid grid-cols-[1.6fr_1.1fr_0.8fr_0.8fr_0.8fr_1fr] items-center px-4 py-4 text-sm text-[#334155]"><span>{item.name}</span><span>{item.code}</span><span>{item.quantity}</span><span>{item.unit}</span><span>{item.unitPrice}</span><span className="text-right font-medium">{formatCurrency(item.totalPrice, item.currencyCode)}</span></div>)}
        </div>
      </div>
