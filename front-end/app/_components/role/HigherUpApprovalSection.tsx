        ...
        <div className="space-y-[12px]">
          {pendingOrders.length > 0 ? pendingOrders.map((order) => (
            <div key={order.id} className="rounded-[10px] border border-[#ececec] px-[14px] py-[14px]">
              ...
              <div className="mt-[12px] grid grid-cols-2 gap-[12px]"><Stat label="Goods count" value={`${order.items.length} type`} /><Stat label="Total amount" value={formatCurrency(order.totalAmount, order.currencyCode)} accent /></div>
